...
use SCMiley\Rss\PrUnderground\RssFeed;
...
$normalFilename = __DIR__ . "/../prUnderground/prUnderground$todayDisplay.csv";
$lowPriorityFilename = __DIR__ . "/../prUnderground/prUnderground{$todayDisplay}_lowPriority.csv";
if(!file_exists($normalFilename)) {
    $fhhp = fopen($normalFilename, 'w');
    $fhlp = fopen($lowPriorityFilename, 'w');

    $header = [
        'URL', 'Company', 'Company URL', 'Title', 'Release Summary', 'Text', 'Contact', 'Email Address', 'Phone Number'
    ];
